...
#Chi-Squared test
chi_table <- rbind( table(as.factor(stratified_q75_CNVs[[3]][1,])), table(as.factor(stratified_q25_CNVs[[3]][1,])))
chisq.test(chi_table, simulate.p.value = TRUE, B = 10000)


gene_name <- leadCandGroups[[3]][1]

list(gene_name, test$p.value)

```

```{R}
# Chi-squared test for a given gene
perform_chisq_test <- function(gene_index) {
    #Combine into single table
  chi_table <- rbind(table( as.factor(stratified_q75_CNVs[[gene_index]][1,])), table(as.factor(stratified_q25_CNVs[[gene_index]][1,])) )
    
    # Chi-squared test
    chisq_result <- chisq.test(chi_table, simulate.p.value = TRUE, B = 10000)
    
    # Get gene name
    gene_name <- leadCandGroups[[gene_index]][1]
    
    # Return gene name and p-value as list
    return(list(
      gene_name, 
